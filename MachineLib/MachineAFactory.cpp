 ...
 */
MachineAFactory::MachineAFactory(std::wstring resourcesDir) :
 ...
 * @return
 */
std::shared_ptr<Machine> MachineAFactory::Create()
{
 // The machine itself
  ...
  */
 auto smile =
     std::make_shared<Smile>(mImagesDir, 212, 80, 80, 15);

 // This simulates the cam key drop so Sparty will immediately pop up
 //sparty->KeyDrop();
 machine->AddComponent(smile);
