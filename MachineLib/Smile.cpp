...
const std::wstring SmileImage = L"/sparty2.png";
/**
* constuctor for smile
...
*/
Smile::Smile(std::wstring imageDir,double size, double springWidth, double springLength, int numLinks):
mImageDir(imageDir), mSpringWidth(springWidth),mSpringLength(springLength),mNumLinks(numLinks)
,mCompressedLength(0), mX(0), mSmileY(-15), mSpringY(-15), mSpringPopped(false)
{
 mSparty.Rectangle(-size/2, mCompressedLength / 2, size, size);
 mSparty.SetImage(mImageDir + SmileImage);
}
...
*/
void Smile::Draw(std::shared_ptr<wxGraphicsContext>  graphics)
{
 ...
 */
void Smile::DrawSpring(std::shared_ptr<wxGraphicsContext> graphics,
