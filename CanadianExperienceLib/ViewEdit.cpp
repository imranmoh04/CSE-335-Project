 ...
 */
ViewEdit::ViewEdit(wxFrame* parent) : wxScrolledCanvas(parent, wxID_ANY), mParentFrame(parent)
{
    ...
    Bind(wxEVT_LEFT_DCLICK, &ViewEdit::OnDoubleClick, this);
 ...
 * @param event
 */
void ViewEdit::OnDoubleClick(wxMouseEvent &event)
{
    // Get the mouse position relative to the canvas
    auto click = CalcUnscrolledPosition(event.GetPosition());

    // Iterate through the actors in the picture to find the drawable under the click
    for (auto actor : *GetPicture())
    {
        // Check if the click hit any drawable of the actor
        auto drawable = actor->HitTest(wxPoint(click.x, click.y));
        if (drawable != nullptr)
        {
           drawable->HandleDoubleClick(mParentFrame);
           UpdateObserver();
           return;
