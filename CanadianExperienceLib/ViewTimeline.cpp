...
void ViewTimeline::OnPaint(wxPaintEvent& event)
{
    ...
    int x = BorderLeft + (int)(timeline->GetTime() * timeline->GetFrameRate() * TickSpacing);
...
void ViewTimeline::OnLeftDown(wxMouseEvent &event)
{
    ...
    // Get the timeline
    Timeline *timeline = GetPicture()->GetTimeline();
    int pointerX = (int)(timeline->GetTime() * timeline->GetFrameRate() * TickSpacing + BorderLeft);
